        ...
        <table className="min-w-[920px] text-left text-sm">
          <thead className="sticky top-0 z-10 bg-white/95 text-neutral-600 backdrop-blur">
            <tr className="border-b border-neutral-200">
              {["Product", "Best For", "Pros", "Cons", "Features", "Amazon CTA"].map((heading) => (
          ...
          <tbody className="divide-y divide-neutral-200">
            {products.map((product) => (
              <tr key={product.id} className="align-top transition hover:bg-neutral-50">
                ...
                <td className="px-4 py-4 text-neutral-700">{product.pros.slice(0, 2).join("; ")}</td>
                <td className="px-4 py-4 text-neutral-700">{product.cons.slice(0, 2).join("; ")}</td>
